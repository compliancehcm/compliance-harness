/**
 * The analysis skill, contributed as an embedded runtime skill.
 *
 ...
 *
 * The body is the real `skills/pandas-analysis/SKILL.md`, read at apply, so the
...
const SKILL_DIR = fileURLToPath(new URL('../skills/pandas-analysis/', import.meta.url))
...
function parseSkillFile(text) {
  const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/.exec(text)
  if (match === null) throw new Error('pandas-analysis skill: SKILL.md has no `---` frontmatter block')
  const fields = {}
  for (const line of match[1].split(/\r?\n/)) {
    if (line.trim() === '') continue
    const pair = /^([A-Za-z][A-Za-z0-9]*): (.+)$/.exec(line)
    if (pair === null) {
      throw new Error(`pandas-analysis skill: frontmatter line is not \`key: value\`: ${JSON.stringify(line)}`)
    }
    const [, key, value] = pair
    if (!KNOWN_KEYS.has(key)) throw new Error(`pandas-analysis skill: unexpected frontmatter key ${JSON.stringify(key)}`)
    if (key in fields) throw new Error(`pandas-analysis skill: duplicate frontmatter key ${JSON.stringify(key)}`)
    fields[key] = value.trim()
  }
  for (const required of ['name', 'description']) {
    if (fields[required] === undefined) throw new Error(`pandas-analysis skill: frontmatter is missing ${required}`)
  }
  ...
  if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(fields.name)) {
    throw new Error(`pandas-analysis skill: name must be kebab-case, got ${JSON.stringify(fields.name)}`)
  }
 ...
 */
export function pandasSkill() {
  const path = `${SKILL_DIR}SKILL.md`
  let text
  try {
    text = readFileSync(path, 'utf8')
  } catch (cause) {
    throw new Error(`pandas-analysis: cannot read the skill body at ${path}`, { cause })
  }
  const { fields, body } = parseSkillFile(text)
  if (body === '') throw new Error('pandas-analysis skill: SKILL.md has an empty body')
  return {
    name: fields.name,
